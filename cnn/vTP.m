...
case {'pool_gradient', 'pool_Jacobian'}
	...
	if strcmp(op, 'pool_gradient')
		num_v = num_data;
		idx = net.idx_pool{m} + [0:num_data-1]*d_prev*a_prev*b_prev;  %( \label{list:vTP|pool-gradient} %)
	else
		num_v = nL*num_data;
		idx = reshape(net.idx_pool{m}, [], 1, num_data) + reshape([0:nL*num_data-1]*d_prev*a_prev*b_prev, 1, nL, num_data);  %( \label{list:vTP|pool-Jacobian} %)
	end
...
vTP = accumarray(idx(:), V(:), [d_prev*a_prev*b_prev*num_v 1])'; %( \label{list:vTP|v} %)
