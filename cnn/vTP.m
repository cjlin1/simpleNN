function vTP = vTP(param, model, net, m, V, op, idx_arr)
...
case {'pool_gradient', 'pool_Jacobian'}
	...
	if strcmp(op, 'pool_gradient')
		num_v = num_data;
		idx = idx_arr + [0:num_data-1]*d_prev*a_prev*b_prev;
	else
		num_v = nL*num_data;
		idx = reshape(idx_arr, [], 1, num_data) + reshape([0:nL*num_data-1]*d_prev*a_prev*b_prev, 1, nL, num_data);
	end
case {'phi_gradient', 'phi_Jacobian'}
	...
	idx = idx_arr(:) + [0:num_v-1]*d_prev*a_prev*b_prev;
otherwise
